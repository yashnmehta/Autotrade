...
void MainWindow::setupContent()
{
    ...
    // Styling is handled inside createConnectionBar via parent/child propagation or shared logic
    // but we can apply the container style here if needed. 
    m_connectionToolBar->setStyleSheet("QToolBar { background-color: #e1e1e1; border: none; border-bottom: 1px solid #d1d1d1; }");
...
void MainWindow::createMenuBar()
{
    ...
    connect(dataMenu->addAction("Start &NSE Broadcast Receiver"), &QAction::triggered, this, &MainWindow::startBroadcastReceiver);
    connect(dataMenu->addAction("St&op NSE Broadcast Receiver"), &QAction::triggered, this, &MainWindow::stopBroadcastReceiver);
...
void MainWindow::createInfoBar()
{
    ...
    connect(m_infoBar, &InfoBar::hideRequested, this, [this]() {
        if (m_infoDock) {
            m_infoDock->hide();
            if (m_infoBarAction) m_infoBarAction->setChecked(false);
    ...
    connect(m_infoDock, &QDockWidget::visibilityChanged, this, [this](bool visible) {
        if (m_infoBarAction) m_infoBarAction->setChecked(visible);
        QSettings s("TradingCompany", "TradingTerminal");
