...
SymbolCacheManager::SymbolCacheManager()
    : QObject(nullptr), m_cacheReady(false) {
  // qDebug() << "[SymbolCacheManager] Constructed (singleton)";
}

SymbolCacheManager::~SymbolCacheManager() { clearCache(); }
...
void SymbolCacheManager::initialize() {
  ...
  if (!repo->isLoaded()) {
    qWarning() << "[SymbolCacheManager] Repository not loaded yet - cannot "
                  "build cache";
    return;
  }
  ...
  // ⚡ OPTIMIZATION: Build caches for segments used by ScripBar
  // Use XTS segment keys ("E" for CM, "F" for FO) to match ScripBar's on-demand
  // lookups
  buildSymbolCache("NSE", "E", "");       // NSE CM (All equity series)
  buildSymbolCache("NSE", "F", "FUTIDX"); // NSE FO Index Futures
  buildSymbolCache("NSE", "F", "FUTSTK"); // NSE FO Stock Futures
  buildSymbolCache("NSE", "O", "OPTIDX"); // NSE FO Index Options
  buildSymbolCache("NSE", "O", "OPTSTK"); // NSE FO Stock Options
  ...
  qDebug() << "[SymbolCacheManager] Total cache entries:"
           << getTotalCacheSize();
...
void SymbolCacheManager::buildSymbolCache(const QString &exchange,
  ...
  /* qDebug() << "[SymbolCacheManager] Found" << contracts.size()
           << "raw contracts for" << key; */
...
const QVector<InstrumentData> &
SymbolCacheManager::getSymbols(const QString &exchange, const QString &segment,
                               const QString &series) {
  QMutexLocker locker(&m_mutex);
  ...
  if (m_symbolCache.contains(key)) {
    /* qDebug() << "[SymbolCacheManager] Cache HIT for" << key << "- Returning"
             << m_symbolCache[key].size() << "entries"; */
    return m_symbolCache[key];
  }

  // Cache miss - try to build it on-demand
  /* qDebug() << "[SymbolCacheManager] Cache MISS for" << key
           << "- Building on-demand..."; */
