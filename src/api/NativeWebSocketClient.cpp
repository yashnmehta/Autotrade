 ...
 * Based on Go reference: trading_terminal_go/internal/services/xts/marketdata.go
 * 
...
struct NativeWebSocketClient::Impl {
    ...
    std::unique_ptr<websocket::stream<ssl::stream<tcp::socket>>> wss;  // SSL WebSocket
                                     ...
                                     OnErrorCallback onError)
{
    ...
    // Parse URL: wss://host:port/path?query or ws://host:port/path?query
    parseUrl(url);
    
    std::cout << "[Native WS] Connecting (" << (m_impl->useSSL ? "SSL" : "plain") << ") to: " 
              << m_impl->host << ":" << m_impl->port << m_impl->path << std::endl;
    ...
    m_impl->ioThread = std::thread([this]() {
        try {
            ...
            if (m_impl->useSSL) {
                ...
                m_impl->wss = std::make_unique<websocket::stream<ssl::stream<tcp::socket>>>(
                ...
                m_impl->wss->set_option(websocket::stream_base::decorator(
                ...
                m_impl->ws = std::make_unique<websocket::stream<tcp::socket>>(m_impl->ioc);
                
                // Connect TCP socket
                net::connect(beast::get_lowest_layer(*m_impl->ws), results);
                
                // Set WebSocket options
                m_impl->ws->set_option(websocket::stream_base::decorator(
                    [](websocket::request_type& req) {
                        req.set(http::field::user_agent, "TradingTerminal/1.0");
                    }));
            ...
            // DO NOT send "40/" Socket.IO connect - Go implementation doesn't send this!
            // The Socket.IO namespace connection happens automatically via Engine.IO handshake
            // Sending "40/" causes duplicate "joined" events and server disconnect
        ...
        // If we should reconnect, spawn a NEW thread (can't reconnect from current thread)
        if (m_shouldReconnect && m_reconnectAttempts < 10) {
            std::thread([this]() {
                // Wait for I/O thread to fully exit
...
void NativeWebSocketClient::disconnect()
{
    ...
    if (m_impl->useSSL && m_impl->wss && m_impl->wss->is_open()) {
        try {
            m_impl->wss->close(websocket::close_code::normal);
        } catch (...) {}
...
{
    if (!m_connected || !m_impl->running) return;
...
void NativeWebSocketClient::sendEngineIOPong()
{
    sendText("3");
}
...
void NativeWebSocketClient::parseEngineIOPacket(const std::string& packet)
{
    ...
    switch (packetType) {
        case '0': { // Engine.IO open (handshake)
            ...
            // Example: {"sid":"...","upgrades":[],"pingInterval":20000,"pingTimeout":60000}
            try {
                ...
                if (pingIntervalPos != std::string::npos) {
                    ...
                    if (numEnd != std::string::npos) {
                        ...
                        std::cout << "📊 Server pingInterval: " << interval << "ms" << std::endl;
                    }
                }
            } catch (...) {
            ...
            // Note: Don't send immediate ping - wait for heartbeat thread to send first ping at 25s interval
            // This matches Go implementation behavior
            break;
        }
...
void NativeWebSocketClient::parseSocketIOEvent(const std::string& payload)
{
    ...
    // Socket.IO event format: "0" for connect, "1" for disconnect, "2[event_name, data]" for event
    if (payload[0] == '0') {
        std::cout << "✅ Socket.IO namespace connected (server confirmed)" << std::endl;
    ...
    if (payload[0] == '1') {
        std::cout << "⚠️ Socket.IO disconnect event (41) received from server" << std::endl;
    ...
    if (payload[0] == '2' && payload.length() > 1) {
        // Socket.IO event - extract JSON array: "2[event_name, data]" -> "[event_name, data]"
...
{
    // Engine.IO Protocol: Client MUST send periodic pings to keep connection alive
    // Verified in working Go implementation: sends "2" (ping) every 25 seconds
    // Server expects this to prove client is still active
    
    m_heartbeatRunning = true;
    m_heartbeatThread = std::thread([this]() {
        std::cout << "💓 Heartbeat started (25s interval, matching Go implementation)" << std::endl;
...
void NativeWebSocketClient::parseUrl(const std::string& url)
{
    ...
    m_impl->path = (pathStart != std::string::npos) ? cleanUrl.substr(pathStart) : "/";
