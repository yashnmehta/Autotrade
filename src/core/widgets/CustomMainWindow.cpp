...
CustomMainWindow::CustomMainWindow(QWidget *parent)
    : QMainWindow(parent), m_centralWidget(nullptr), m_isMaximized(false),
      m_isResizing(false), m_resizeDirection(None), m_pendingResize(false),
      m_pendingResizeStartPos(QPoint()), m_pendingResizeDirection(None),
      m_minimumSize(400, 300), m_maximumSize(QWIDGETSIZE_MAX, QWIDGETSIZE_MAX) {
  setupUi();
  applyDefaultStyling();
...
void CustomMainWindow::setupUi() {
  ...
  m_titleBar->setActive(true); // Ensure it starts with active styling
...
void CustomMainWindow::setCentralWidget(QWidget *widget) {
  ...
  // We use a wrapper widget to hold both our custom header and the user's
  // central widget. This avoids issues with QMainWindow::setMenuWidget on
  // frameless macOS windows.
...
void CustomMainWindow::addCustomToolBar(QWidget *toolbar) {
  ...
  if (qobject_cast<QToolBar *>(toolbar)) {
    ...
    // Fallback or wrapper for non-QToolBar widgets - maybe add to a default
    // toolbar?
...
QString CustomMainWindow::title() const { return m_titleBar->title(); }
...
void CustomMainWindow::showMinimized() { QWidget::showMinimized(); }
...
void CustomMainWindow::mousePressEvent(QMouseEvent *event) {
  if (event->button() == Qt::LeftButton && !m_isMaximized) {
    ...
    qDebug() << "[MainWindow Press] pos:" << event->pos()
             << "globalPos:" << event->globalPos()
             << "detected direction:" << m_resizeDirection
             << "windowSize:" << size();
    ...
    if (m_resizeDirection != None) {
      ...
      qDebug() << "[MainWindow] Resize started, direction:"
               << m_resizeDirection;
      return;
    }
  }
...
CustomMainWindow::ResizeDirection
CustomMainWindow::detectResizeDirection(const QPoint &pos) const {
  int direction = None;
...
void CustomMainWindow::saveNormalGeometry() { m_normalGeometry = geometry(); }
...
bool CustomMainWindow::eventFilter(QObject *watched, QEvent *event) {
  ...
  // Also handle events from title bar children (buttons)
  else if (m_titleBar->isAncestorOf(qobject_cast<QWidget *>(watched)) &&
    ...
    if (mouseEvent->button() == Qt::LeftButton && !m_isMaximized) {
      ...
      qDebug() << "[Press Child]" << childWidget->objectName()
               << "childPos:" << mouseEvent->pos() << "windowPos:" << windowPos
               << "direction:" << direction;
    ...
    if (m_isResizing && (mouseEvent->buttons() & Qt::LeftButton)) {
      ...
      ResizeDirection direction = detectResizeDirection(windowPos);
      updateCursorShape(direction);
      // do NOT return true; let title bar handle the underlying event
      // (dragging)
    }
  }
  // Handle move events from title bar children
    ...
    if (m_isResizing && (mouseEvent->buttons() & Qt::LeftButton)) {
      ...
      updateCursorShape(direction);
              ...
              m_titleBar->isAncestorOf(qobject_cast<QWidget *>(watched))) &&
