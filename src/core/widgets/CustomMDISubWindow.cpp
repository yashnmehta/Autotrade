...
CustomMDISubWindow::CustomMDISubWindow(const QString &title, QWidget *parent)
    : QWidget(parent), m_contentWidget(nullptr), m_isMinimized(false),
      m_isMaximized(false), m_isPinned(false), m_isCached(false),
  ...
  connect(m_titleBar, &CustomTitleBar::minimizeClicked, this,
          &CustomMDISubWindow::minimizeRequested);
  ...
  connect(m_titleBar, &CustomTitleBar::dragStarted, this,
          [this](const QPoint &globalPos) {
            ...
            // If the press started on the resize border (top edge inside title
            // bar), begin resize
            ...
            if (isOnResizeBorder(local, edges)) {
              m_isResizing = true;
              m_resizeEdges = edges;
              qDebug() << "[MDISubWindow] start resize from titlebar, edges:"
                       << edges;
            }
          });
  ...
  connect(m_titleBar, &CustomTitleBar::dragMoved, this,
          [this](const QPoint &globalPos) {
            if (m_isMaximized || m_isPinned)
              return; // Don't move/resize if maximized or pinned
            ...
            if (m_isResizing) {
              ...
              CustomMDIArea *mdiArea =
                  qobject_cast<CustomMDIArea *>(parentWidget());
              if (mdiArea) {
...
void CustomMDISubWindow::closeEvent(QCloseEvent *event) {
  ...
  if (m_isCached) {
    ...
    move(WindowConstants::OFF_SCREEN_X, WindowConstants::OFF_SCREEN_Y);
...
void CustomMDISubWindow::keyPressEvent(QKeyEvent *event) {
  ...
  if (event->key() == Qt::Key_F1 || event->key() == Qt::Key_F2) {
    ...
    if (topLevel) {
      // Pass the content widget as initiating window so focus returns here
      const char *method = (event->key() == Qt::Key_F1)
                               ? "createBuyWindowFromWidget"
                               : "createSellWindowFromWidget";
      bool ok = QMetaObject::invokeMethod(topLevel, method,
...
QString CustomMDISubWindow::title() const { return m_titleBar->title(); }
...
void CustomMDISubWindow::maximize() {
  qDebug() << "[MDISubWindow] maximize() called for" << title()
...
void CustomMDISubWindow::mousePressEvent(QMouseEvent *event) {
  if (event->button() == Qt::LeftButton) {
    qDebug() << "[MDISubWindow]" << title() << "mousePressEvent at"
             << event->pos();
    emit windowActivated();
    ...
    if (isOnResizeBorder(event->pos(), edges)) {
      qDebug() << "[MDISubWindow]" << title()
               << "starting resize, edges:" << edges;
      m_isResizing = true;
      m_resizeEdges = edges;
...
void CustomMDISubWindow::mouseMoveEvent(QMouseEvent *event) {
  if (m_isResizing && (event->buttons() & Qt::LeftButton)) {
    ...
    if (newGeometry.width() < minW)
      newGeometry.setWidth(minW);
    if (newGeometry.height() < minH)
      newGeometry.setHeight(minH);
...
bool CustomMDISubWindow::isOnResizeBorder(const QPoint &pos,
...
void CustomMDISubWindow::contextMenuEvent(QContextMenuEvent *event) {
  ...
  QAction *pinAction =
      menu.addAction(m_isPinned ? "Unpin Window" : "Pin Window");
  menu.addSeparator();

  QAction *minimizeAction = menu.addAction("Minimize");
  QAction *maximizeAction =
      menu.addAction(m_isMaximized ? "Restore" : "Maximize");
  menu.addSeparator();
...
void CustomMDISubWindow::paintEvent(QPaintEvent *event) {
  ...
  if (!m_isMaximized && m_titleBar &&
      m_titleBar->property("isActive").toBool()) {
    painter.setBrush(borderColor);
    int handleSize = 10;
    // Bottom right corner handle
    QRect brHandle(width() - handleSize, height() - handleSize, handleSize,
                   handleSize);
