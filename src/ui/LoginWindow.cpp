    ...
    , m_onContinueCallback(nullptr)
{
    ...
    // Connect signals
    connect(ui->pbLogin, &QPushButton::clicked, this, &LoginWindow::onLoginButtonClicked);
    connect(ui->pbNext, &QPushButton::clicked, this, &LoginWindow::onContinueButtonClicked);
    connect(ui->pbExit, &QPushButton::clicked, this, &LoginWindow::onExitButtonClicked);
}
...
void LoginWindow::setLoginID(const QString &id)
{
    ui->lb_loginId->setText(id);
}
...
void LoginWindow::disableLoginButton()
{
...
void LoginWindow::onLoginButtonClicked()
{
    ...
    if (getMarketDataAppKey().isEmpty() || getMarketDataSecretKey().isEmpty()) {
        QMessageBox::warning(this, "Validation Error", "Market Data credentials are required");
        return;
    }
    
    if (getInteractiveAppKey().isEmpty() || getInteractiveSecretKey().isEmpty()) {
        QMessageBox::warning(this, "Validation Error", "Interactive API credentials are required");
        return;
    }
