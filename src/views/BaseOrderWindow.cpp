...
void BaseOrderWindow::setupBaseConnections() {
  if (m_pbSubmit)
    connect(m_pbSubmit, &QPushButton::clicked, this,
            &BaseOrderWindow::onSubmitClicked);
  if (m_pbClear)
    connect(m_pbClear, &QPushButton::clicked, this,
            &BaseOrderWindow::onClearClicked);
  if (m_pbAMO)
    connect(m_pbAMO, &QPushButton::clicked, this,
            &BaseOrderWindow::onAMOClicked);
  if (m_cbOrdType) {
    connect(m_cbOrdType,
  ...
  if (m_leQty) {
    ...
    qDebug() << "[BaseOrderWindow] Installed event filter and validator on "
                "Quantity field";
  }
  if (m_leDiscloseQty) {
  ...
  if (m_leRate) {
    ...
    QDoubleValidator *priceValidator =
        new QDoubleValidator(0.0, 10000000.0, 2, this);
  ...
  if (m_leTrigPrice) {
    QDoubleValidator *trigValidator =
        new QDoubleValidator(0.0, 10000000.0, 2, this);
...
void BaseOrderWindow::populateBaseComboBoxes() {
  ...
  if (m_cbInstrName)
    m_cbInstrName->addItems({"FUTIDX", "FUTSTK", "OPTIDX", "OPTSTK", "EQ"});
  if (m_cbOrdType)
    m_cbOrdType->addItems({"Limit", "Market", "StopLimit", "StopMarket"});
  if (m_cbOrderType2)
  ...
  if (m_cbProduct)
    m_cbProduct->addItems(
        {"MIS", "NRML", "CNC", "CO", "BO"}); // API compatible codes
  if (m_cbValidity)
...
void BaseOrderWindow::loadBasePreferences() {
  ...
  if (m_cbProduct) {
    ...
    if (defProd == "INTRADAY")
      defProd = "MIS";
    else if (defProd == "DELIVERY")
      defProd = "NRML";

    int idx = m_cbProduct->findText(defProd);
    if (idx < 0)
      idx = m_cbProduct->findText("MIS");
    if (idx >= 0)
...
void BaseOrderWindow::cacheLineEdits() {
  ...
  qDebug() << "[BaseOrderWindow] Cached" << m_cachedLineEdits.size()
           << "line edits for fast clearing";
}
...
void BaseOrderWindow::applyDefaultFocus() {
  ...
  if (targetWidget) {
    ...
    if (le) {
      ...
      qDebug() << "[BaseOrderWindow] Applied focus to:"
               << targetWidget->objectName() << "Value:" << le->text()
               << "Cursor at end, ready for editing";
    }
  } else {
...
void BaseOrderWindow::setScripDetails(const QString &exchange, int token,
...
void BaseOrderWindow::loadFromContext(const WindowContext &context) {
  ...
  if (m_cbInstrName) {
    ...
    if (idx >= 0) {
      ...
      if (m_cbInstrName->isEditable()) {
        ...
        qDebug() << "[BaseOrderWindow] Instrument Type"
                 << context.instrumentType << "not found in dropdown";
      }
    }
  }
  if (m_leQty) {
  ...
  // NOTE: Focus is applied asynchronously by WindowCacheManager after window is
  // shown Applying it here causes black screen rendering issues
  ...
  if (m_cbOptType) {
    ...
    if (idx >= 0) {
      ...
      // If not found within CE/PE (e.g. FUT, XX), clear selection
      // Users requested "XX" or " " for Futures
      m_cbOptType->setCurrentIndex(-1);
    }
  }
...
void BaseOrderWindow::loadFromOrder(const XTS::Order &order) {
  ...
  if (m_leToken)
    m_leToken->setText(QString::number(order.exchangeInstrumentID));
  if (m_leSymbol)
    m_leSymbol->setText(order.tradingSymbol);
  ...
  qDebug() << "[BaseOrderWindow] Loaded order for modification. AppOrderID:"
           << order.appOrderID << "Symbol:" << order.tradingSymbol
           << "Qty:" << order.orderQuantity << "Price:" << order.orderPrice;
}
...
void BaseOrderWindow::loadFromOrders(const QVector<XTS::Order> &orders) {
  ...
  if (!compatible) {
    qWarning()
        << "[BaseOrderWindow] Incompatible orders for batch modification";
    return;
  }
  ...
  setWindowTitle(QString("Batch Modify %1 Order (%2 orders)")
                     .arg(type)
                     .arg(orders.size()));

  qDebug() << "[BaseOrderWindow] Loaded batch of" << orders.size()
           << "orders for modification.";
}
...
void BaseOrderWindow::setModifyMode(bool enabled) {
  ...
  if (m_cbEx)
    m_cbEx->setEnabled(!enabled);
  if (m_leToken)
    m_leToken->setEnabled(!enabled);
  if (m_leSymbol)
    m_leSymbol->setEnabled(!enabled);
  if (m_leInsType)
    m_leInsType->setEnabled(!enabled);
  if (m_cbInstrName)
    m_cbInstrName->setEnabled(!enabled);
  if (m_cbExp)
    m_cbExp->setEnabled(!enabled);
  if (m_cbStrike)
    m_cbStrike->setEnabled(!enabled);
  if (m_cbOptType)
    m_cbOptType->setEnabled(!enabled);
  if (m_cbProduct)
    m_cbProduct->setEnabled(!enabled); // Product type cannot be modified
  if (m_cbProCli)
    m_cbProCli->setEnabled(!enabled);
  if (m_cbOC)
    m_cbOC->setEnabled(!enabled);

  // Editable fields remain enabled:
  // - Quantity (m_leQty) - Disabled in Batch Mode to prevent accidental mass
  // change
  ...
  if (m_leQty)
    m_leQty->setEnabled(!isBatch);
  if (m_leDiscloseQty)
    m_leDiscloseQty->setEnabled(!isBatch);

  // Update submit button text
  if (m_pbSubmit) {
    if (isBatch)
      m_pbSubmit->setText(
          QString("Modify %1 Orders").arg(m_batchOrders.size()));
    else
  ...
  if (enabled && !isBatch) {
    setWindowTitle(windowTitle()
                       .replace("Buy", "Modify Buy")
                       .replace("Sell", "Modify Sell"));
  }
}
  ...
  if (event->type() == QEvent::KeyPress) {
    ...
    if (obj == m_leQty) {
      ...
      if (keyEvent->key() == Qt::Key_Plus || keyEvent->key() == Qt::Key_Minus) {
        qDebug() << "[BaseOrderWindow] +/- key pressed in Quantity field - "
                    "switching window";
      ...
      if (keyEvent->key() == Qt::Key_Up || keyEvent->key() == Qt::Key_Down) {
        qDebug() << "[BaseOrderWindow] Arrow key pressed in Quantity field:"
                 << (keyEvent->key() == Qt::Key_Up ? "UP" : "DOWN");
    ...
    // Handle Price field
      ...
      // Intercept +/- keys BEFORE validator processes them
      if (keyEvent->key() == Qt::Key_Plus || keyEvent->key() == Qt::Key_Minus) {
        qDebug() << "[BaseOrderWindow] +/- key pressed in Price field - "
                    "switching window";
      ...
      if (keyEvent->key() == Qt::Key_Up || keyEvent->key() == Qt::Key_Down) {
        qDebug() << "[BaseOrderWindow] Arrow key pressed in Price field:"
                 << (keyEvent->key() == Qt::Key_Up ? "UP" : "DOWN");
