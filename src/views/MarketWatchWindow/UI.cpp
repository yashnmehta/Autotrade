 ...
 *
 * Ensures that background colors provided by the model (e.g., for LTP/Bid/Ask
 * ticks) take precedence over the standard selection highlighting.
 */
...
public:
  ...
  void paint(QPainter *painter, const QStyleOptionViewItem &option,
    ...
    if (bg.isValid()) {
      // Tick flash: fill with model color, dark text on light bg, suppress
      // selection highlight
...
void MarketWatchWindow::setupUI() {
  ...
  connect(horizontalHeader(), &QHeaderView::customContextMenuRequested, this,
          [this](const QPoint &headerPos) {
            // Map header-local pos into viewport coords so indexAt /
            // mapToGlobal work correctly
            QPoint viewportPos = viewport()->mapFromGlobal(
                horizontalHeader()->mapToGlobal(headerPos));
...
void MarketWatchWindow::showContextMenu(const QPoint &pos) {
  ...
  // Base Portfolio action
  menu.addAction("Make Delta Portfolio", this,
                 &MarketWatchWindow::onMakeDeltaPortfolio);
  menu.addSeparator();

  // Blank row insertion
  if (proxyIndex.isValid() && sourceRow >= 0) {
    menu.addAction("Insert Blank Row Above", this,
                   [this, sourceRow]() { insertBlankRow(sourceRow); });
    menu.addAction("Insert Blank Row Below", this,
                   [this, sourceRow]() { insertBlankRow(sourceRow + 1); });
  } else {
    menu.addAction("Insert Blank Row", this,
                   [this]() { insertBlankRow(m_model->rowCount()); });
  }

  // Delete and clipboard operations
  if (proxyIndex.isValid()) {
    menu.addSeparator();
    menu.addAction("Delete (Del)", this,
                   &MarketWatchWindow::deleteSelectedRows);
    menu.addSeparator();
  ...
  menu.addAction("Paste (Ctrl+V)", this,
                 &MarketWatchWindow::pasteFromClipboard);

  // Column profile
  menu.addSeparator();
  menu.addAction("Column Profile...", this,
                 &MarketWatchWindow::showColumnProfileDialog);

  // Portfolio Management
  menu.addSeparator();
  menu.addAction("Save Portfolio...", this,
                 &MarketWatchWindow::onSavePortfolio);
  menu.addAction("Load Portfolio...", this,
                 &MarketWatchWindow::onLoadPortfolio);
...
void MarketWatchWindow::showColumnProfileDialog() {
  ...
  GenericProfileDialog dialog("Market Watch \u2014 Column Profile", metadata,
                              &mgr, m_model->getColumnProfile(), this);
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    qDebug() << "[MarketWatchWindow] Column profile updated to:"
             << newProfile.name();
  }
}
