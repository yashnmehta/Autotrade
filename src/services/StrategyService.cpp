    ...
    : QObject(nullptr)
{
    ...
    connect(&m_updateTimer, &QTimer::timeout, this, &StrategyService::onUpdateTick);
}
...
qint64 StrategyService::createInstance(const QString& name,
...
bool StrategyService::startStrategy(qint64 instanceId)
{
    ...
    // State validation: only start from Created or Stopped
    if (instance->state != StrategyState::Created && instance->state != StrategyState::Stopped) {
        qDebug() << "[StrategyService] Cannot start from state" << StrategyInstance::stateToString(instance->state);
        return false;
    }
    ...
    if (ok) {
        emit metricsUpdated(instance->instanceId, instance->mtm, instance->stopLoss, instance->target);
    }
    return ok;
}
...
bool StrategyService::pauseStrategy(qint64 instanceId)
{
    ...
    // Pause only allowed from Running state
    if (instance->state != StrategyState::Running) {
        qDebug() << "[StrategyService] Cannot pause from state" << StrategyInstance::stateToString(instance->state);
        return false;
    }
    return updateState(*instance, StrategyState::Paused);
}
...
bool StrategyService::resumeStrategy(qint64 instanceId)
{
    ...
    // Resume only allowed from Paused state
    if (instance->state != StrategyState::Paused) {
        qDebug() << "[StrategyService] Cannot resume from state" << StrategyInstance::stateToString(instance->state);
        return false;
    }
    return updateState(*instance, StrategyState::Running);
}
...
bool StrategyService::stopStrategy(qint64 instanceId)
{
    ...
    // Stop allowed from Running or Paused
    if (instance->state != StrategyState::Running && instance->state != StrategyState::Paused) {
        qDebug() << "[StrategyService] Cannot stop from state" << StrategyInstance::stateToString(instance->state);
        return false;
    }
    ...
    return updateState(*instance, StrategyState::Stopped);
}
...
bool StrategyService::deleteStrategy(qint64 instanceId)
{
    ...
    if (instance->state != StrategyState::Stopped) {
        qDebug() << "[StrategyService] Cannot delete from state" << StrategyInstance::stateToString(instance->state) << "(only from Stopped)";
        return false;
    }
                                       ...
                                       QString* outError)
{
    StrategyInstance* instance = findInstance(instanceId);
    if (!instance) {
        if (outError) *outError = "Instance not found";
        return false;
    }
    ...
    if (instance->state == StrategyState::Running) {
        for (const QString& key : instance->lockedParameters) {
            if (parameters.contains(key) && parameters[key] != instance->parameters.value(key)) {
                if (outError) *outError = QString("Parameter locked while running: %1").arg(key);
...
bool StrategyService::updateMetrics(qint64 instanceId,
                                    double mtm,
                                    int activePositions,
                                    int pendingOrders)
{
...

    {
        ...
        for (auto it = m_instances.begin(); it != m_instances.end(); ++it) {
            ...
            if (instance.state == StrategyState::Running) {
                ...
                // - Formula: MTM = instance.quantity * (currentPrice - instance.entryPrice)
...
bool StrategyService::updateState(StrategyInstance& instance, StrategyState newState)
{
