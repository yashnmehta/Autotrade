...
UDP::MarketTick convertNseFoUnified(
  ...
  // Depth
  for (int i = 0; i < 5; i++) {
    tick.bids[i] = UDP::DepthLevel(data.bids[i].price, data.bids[i].quantity,
                                   data.bids[i].orders);
    tick.asks[i] = UDP::DepthLevel(data.asks[i].price, data.asks[i].quantity,
                                   data.asks[i].orders);
  }
...
UDP::SessionStateTick
convertBseSessionState(const bse::DecodedSessionState &state,
                       UDP::ExchangeSegment segment) {
  ...
  case 0:
    sessTick.state = UDP::SessionState::PRE_OPEN;
    break;
  case 1:
    sessTick.state = UDP::SessionState::CONTINUOUS;
    break;
  case 2:
    sessTick.state = UDP::SessionState::AUCTION;
    break;
  default:
    sessTick.state = UDP::SessionState::UNKNOWN;
    break;
  }
...
UDP::CircuitLimitTick convertBseCircuitLimit(uint32_t token,
...
UDP::IndexTick convertNseFoIndex(const nsefo::IndexData &data) {
  ...
  if (sizeof(data.name) < copySize)
    copySize = sizeof(data.name);
...
UDP::IndexTick convertNseFoIndustryIndex(const nsefo::IndustryIndexData &data) {
  ...
  if (sizeof(data.name) < copySize2)
    copySize2 = sizeof(data.name);
...
UDP::IndexTick convertBseIndex(const bse::DecodedRecord &record,
                               UDP::ExchangeSegment segment) {
  UDP::IndexTick tick;
  tick.exchangeSegment = segment;
  ...
  tick.change = tick.value - tick.prevClose;
  tick.changePercent =
      (tick.prevClose > 0) ? (tick.change / tick.prevClose) * 100.0 : 0.0;
...
UDP::CircuitLimitTick
convertNseFoCircuitLimit(const nsefo::CircuitLimitData &data) {
  UDP::CircuitLimitTick tick;
  tick.exchangeSegment = UDP::ExchangeSegment::NSEFO;
...
void UdpBroadcastService::subscribeToken(uint32_t token, int exchangeSegment) {
  ...
  MarketData::PriceStoreGateway::instance().setTokenEnabled(exchangeSegment,
                                                            token, true);
...
void UdpBroadcastService::unsubscribeToken(uint32_t token,
                                           int exchangeSegment) {
  std::unique_lock lock(m_subscriptionMutex);
...
void UdpBroadcastService::setSubscriptionFilterEnabled(bool enabled) {
  ...
  qDebug() << "[UdpBroadcast] Subscription filtering:"
           << (enabled ? "ENABLED" : "DISABLED");
  if (!enabled) {
    qWarning() << "[UdpBroadcast] Warning: Filtering disabled - will emit "
                  "signals for ALL ticks (high CPU usage)";
  }
}
...
void UdpBroadcastService::setupNseFoCallbacks() {
  ...
  nsefo::MarketDataCallbackRegistry::instance().setTokenFilter(
      [](int32_t token) {
        return MarketData::PriceStoreGateway::instance().isTokenEnabled(2,
                                                                        token);
      });
  ...
  nsefo::MarketDataCallbackRegistry::instance().registerIndustryIndexCallback(
      [this](const nsefo::IndustryIndexData &data) {
        // Emit new UDP::IndexTick
...
void UdpBroadcastService::setupNseCmCallbacks() {
  // 0. Set Notification Filter
  nsecm::MarketDataCallbackRegistry::instance().setTokenFilter(
      [](int32_t token) {
        return MarketData::PriceStoreGateway::instance().isTokenEnabled(1,
                                                                        token);
      });
  ...
  nsecm::MarketDataCallbackRegistry::instance().registerMarketWatchCallback(
      [this](const nsecm::MarketWatchData &data) {
  ...
  nsecm::MarketDataCallbackRegistry::instance().registerAdminCallback(
      [this](const nsecm::AdminMessage &data) {
        // Log admin messages (can be removed in production)
        // qDebug() << "NSE CM Admin: Action=" <<
        // QString::fromStdString(data.actionCode)
        //          << " Msg=" << QString::fromStdString(data.message);
      });

  // System information (7206)
  nsecm::MarketDataCallbackRegistry::instance()
      .registerSystemInformationCallback(
          [this](const nsecm::SystemInformationData &data) {
            // Log system information (can be removed in production)
            // qDebug() << "NSE CM System Info: MarketIndex=" <<
            // data.marketIndex
  ...
  nsecm::MarketDataCallbackRegistry::instance()
      .registerCallAuctionOrderCxlCallback(
          [this](const nsecm::CallAuctionOrderCxlData &data) {
            // Log call auction cancellations (can be removed in production)
            // qDebug() << "NSE CM Call Auction Cxl: Records=" <<
            // data.noOfRecords;
          });

  // Market open/close/pre-open messages (6511, 6521, 6522, 6531, 6571)
  nsecm::MarketDataCallbackRegistry::instance().registerMarketOpenCallback(
      [this](const nsecm::MarketOpenMessage &data) {
...
void UdpBroadcastService::setupBseFoCallbacks() {
  ...
  m_bseFoReceiver->setSessionStateCallback(
      [this](const bse::DecodedSessionState &state) {
        UDP::SessionStateTick sessTick =
            convertBseSessionState(state, UDP::ExchangeSegment::BSEFO);
...
void UdpBroadcastService::setupBseCmCallbacks() {
  ...
  m_bseCmReceiver->setSessionStateCallback(
      [this](const bse::DecodedSessionState &state) {
        UDP::SessionStateTick sessTick =
            convertBseSessionState(state, UDP::ExchangeSegment::BSECM);
        emit udpSessionStateReceived(sessTick);
      });
...
void UdpBroadcastService::start(const Config &config) {
  ...
  try {
    ...
    if (config.enableNSEFO && !config.nseFoIp.empty() && config.nseFoPort > 0) {
      if (startReceiver(ExchangeReceiver::NSEFO, config.nseFoIp,
                        config.nseFoPort)) {
        anyEnabled = true;
      }
      if (anyEnabled)
    ...
    if (config.enableNSECM && !config.nseCmIp.empty() && config.nseCmPort > 0) {
      if (startReceiver(ExchangeReceiver::NSECM, config.nseCmIp,
                        config.nseCmPort)) {
        anyEnabled = true;
      }
      if (anyEnabled)
    ...
    if (config.enableBSEFO && !config.bseFoIp.empty() && config.bseFoPort > 0) {
      if (startReceiver(ExchangeReceiver::BSEFO, config.bseFoIp,
                        config.bseFoPort)) {
        anyEnabled = true;
      }
      if (anyEnabled)
    ...
    if (config.enableBSECM && !config.bseCmIp.empty() && config.bseCmPort > 0) {
      if (startReceiver(ExchangeReceiver::BSECM, config.bseCmIp,
                        config.bseCmPort)) {
        anyEnabled = true;
      }
    }
...
bool UdpBroadcastService::startReceiver(ExchangeReceiver receiver,
  ...
  try {
    ...
    case ExchangeReceiver::NSEFO: {
      ...
      if (m_nseFoReceiver->isValid()) {
        ...
        qDebug() << "  -> NSE FO: STARTED on" << QString::fromStdString(ip)
                 << ":" << port;
        return true;
      }
      break;
    ...
    case ExchangeReceiver::NSECM: {
      ...
      if (m_nseCmReceiver->isValid()) {
        ...
        qDebug() << "  -> NSE CM: STARTED on" << QString::fromStdString(ip)
                 << ":" << port;
        return true;
      }
      break;
    ...
    case ExchangeReceiver::BSEFO: {
      ...
      qDebug() << "  -> BSE FO: STARTED on" << QString::fromStdString(ip) << ":"
               << port;
      return true;
    }
    ...
    case ExchangeReceiver::BSECM: {
      ...
      qDebug() << "  -> BSE CM: STARTED on" << QString::fromStdString(ip) << ":"
               << port;
      return true;
    }
    default:
...
bool UdpBroadcastService::isReceiverActive(ExchangeReceiver receiver) const {
  switch (receiver) {
  case ExchangeReceiver::NSEFO:
    return m_nseFoActive;
  case ExchangeReceiver::NSECM:
    return m_nseCmActive;
  case ExchangeReceiver::BSEFO:
...
void UdpBroadcastService::stop() {
  if (!m_active && !m_nseFoActive && !m_nseCmActive && !m_bseFoActive &&
      !m_bseCmActive)
    return;
...
bool UdpBroadcastService::isActive() const { return m_active; }

UdpBroadcastService::Stats UdpBroadcastService::getStats() const {
  Stats s;
  s.nseFoPackets =
      m_nseFoReceiver ? m_nseFoReceiver->getStats().totalPackets : 0;
  s.nseCmPackets =
      m_nseCmReceiver ? m_nseCmReceiver->getStats().totalPackets : 0;
  s.bseFoPackets =
      m_bseFoReceiver ? m_bseFoReceiver->getStats().packetsReceived : 0;
  s.bseCmPackets =
      m_bseCmReceiver ? m_bseCmReceiver->getStats().packetsReceived : 0;
