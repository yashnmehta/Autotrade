...
StrategyRepository::StrategyRepository(QObject *parent)
    : QObject(parent),
      m_connectionName("strategy_manager_db")
{
}
...
bool StrategyRepository::open(const QString& dbPath)
{
    ...
    if (!m_db.open()) {
        qDebug() << "[StrategyRepository] Failed to open database:" << m_db.lastError().text();
        return false;
    }
...
bool StrategyRepository::isOpen() const
{
    return m_db.isOpen();
}
...
bool StrategyRepository::ensureSchema()
{
    ...
    if (!query.exec(sql)) {
        qDebug() << "[StrategyRepository] Schema creation failed:" << query.lastError().text();
        return false;
    }
...
qint64 StrategyRepository::saveInstance(StrategyInstance& instance)
{
    ...
    QSqlQuery query(m_db);
    query.prepare(
        "INSERT INTO strategy_instances (instance_name, strategy_type, symbol, state, mtm, stop_loss, target, entry_price, "
        "quantity, active_positions, pending_orders, parameters_json, created_at, last_updated, last_state_change, start_time, last_error, deleted) "
        "VALUES (:instance_name, :strategy_type, :symbol, :state, :mtm, :stop_loss, :target, :entry_price, :quantity, :active_positions, "
        ":pending_orders, :parameters_json, :created_at, :last_updated, :last_state_change, :start_time, :last_error, 0)"
    );
    ...
    if (!query.exec()) {
        qDebug() << "[StrategyRepository] Insert failed:" << query.lastError().text();
        return 0;
    }
...
bool StrategyRepository::updateInstance(const StrategyInstance& instance)
{
    ...
    query.prepare(
        ...
        "instance_name = :instance_name, strategy_type = :strategy_type, symbol = :symbol, state = :state, mtm = :mtm, "
        "stop_loss = :stop_loss, target = :target, entry_price = :entry_price, quantity = :quantity, active_positions = :active_positions, "
        "pending_orders = :pending_orders, parameters_json = :parameters_json, created_at = :created_at, last_updated = :last_updated, "
        "last_state_change = :last_state_change, start_time = :start_time, last_error = :last_error "
    ...
    if (!query.exec()) {
        qDebug() << "[StrategyRepository] Update failed:" << query.lastError().text();
        return false;
    }
...
bool StrategyRepository::markDeleted(qint64 instanceId)
{
    ...
    QSqlQuery query(m_db);
    query.prepare("UPDATE strategy_instances SET deleted = 1, state = 'DELETED' WHERE instance_id = :instance_id");
    query.bindValue(":instance_id", instanceId);

    if (!query.exec()) {
        qDebug() << "[StrategyRepository] Delete failed:" << query.lastError().text();
        return false;
    }
...
QVector<StrategyInstance> StrategyRepository::loadAllInstances(bool includeDeleted)
{
...
StrategyInstance StrategyRepository::fromQuery(QSqlQuery& query) const
{
    ...
    instance.state = StrategyInstance::stringToState(query.value("state").toString());
    ...
    instance.lastStateChange = fromIsoString(query.value("last_state_change").toString());
