    ...
    const StrategyTemplate &tmpl) const
{
    ...
    for (const auto &s : tmpl.symbols) {
        ...
        o["segment"]   = (s.segment == ExchangeSegment::NSEFO) ? "nse_fo"
                       : (s.segment == ExchangeSegment::BSECM) ? "bse_cm"
                       : (s.segment == ExchangeSegment::BSEFO) ? "bse_fo"
                                                                : "nse_cm";
    ...
    const QString &updatedAt) const
{
    ...
    for (const auto &sv : root["symbols"].toArray()) {
        ...
        s.segment = (seg == "nse_fo") ? ExchangeSegment::NSEFO
                  : (seg == "bse_cm") ? ExchangeSegment::BSECM
                  : (seg == "bse_fo") ? ExchangeSegment::BSEFO
