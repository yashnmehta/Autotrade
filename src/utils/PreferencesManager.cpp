...
PreferencesManager::PreferencesManager()
  ...
  qDebug() << "[PreferencesManager] Initialized with config:"
           << m_settings.fileName();
}
...
bool PreferencesManager::getUseLegacyPriceCache() const {
  // Default to true = real-time UDP push path (sub-ms per-tick updates).
  // false = zero-copy 100ms polling timer (only for testing/verification).
...
PreferencesManager::FocusField
PreferencesManager::getOrderWindowFocusField() const {
  QString fieldStr =
      m_settings.value("order/focus_field", "Quantity").toString();
...
QString PreferencesManager::focusFieldToString(FocusField field) const {
  switch (field) {
  case FocusField::Quantity:
    return "Quantity";
  case FocusField::Price:
    return "Price";
  case FocusField::Scrip:
    return "Scrip";
  default:
    return "Quantity";
  }
...
PreferencesManager::FocusField
PreferencesManager::stringToFocusField(const QString &str) const {
  if (str == "Price")
    return FocusField::Price;
  if (str == "Scrip")
    return FocusField::Scrip;
...
QVariant
PreferencesManager::getWindowPreference(const QString &window,
                                        const QString &key,
...
PreferencesManager::TradingPreset
PreferencesManager::getPreset(const QString &name) const {
  TradingPreset preset;
  QString prefix = QString("presets/%1/").arg(name);

  preset.orderType =
      m_settings.value(prefix + "order_type", DEFAULT_ORDER_TYPE).toString();
  preset.product =
      m_settings.value(prefix + "product", DEFAULT_PRODUCT).toString();
  preset.validity =
      m_settings.value(prefix + "validity", DEFAULT_VALIDITY).toString();
  preset.quantity =
      m_settings.value(prefix + "quantity", DEFAULT_QUANTITY).toInt();
...
void PreferencesManager::savePreset(const QString &name,
                                    const TradingPreset &preset) {
  QString prefix = QString("presets/%1/").arg(name);
