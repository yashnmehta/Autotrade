...
/**
 * @brief Alias to the canonical ::ExchangeSegment (core/ExchangeSegment.h)
 *
 * Values ARE the XTS API codes — 1,2,3,11,12,51,61 — because XTS is the
 * authoritative source.  The canonical enum lives in core/ so every module
 * (UDP, Strategy, Repository, Feed) shares a single definition with no
 * conversion.  Existing code using XTS::ExchangeSegment::NSEFO etc.
 * compiles unchanged.
 */
