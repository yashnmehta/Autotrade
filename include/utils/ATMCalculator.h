...
public:
     ...
     */
    static CalculationResult calculateFromActualStrikes(double basePrice, const QVector<double>& actualStrikes, int rangeCount = 0) {
        CalculationResult result;
        if (actualStrikes.isEmpty() || basePrice <= 0) return result;
        ...
        // Find nearest strike
        auto it = std::lower_bound(sortedStrikes.begin(), sortedStrikes.end(), basePrice);
        ...
        if (rangeCount > 0) {
            ...
            int endIdx = std::min((int)sortedStrikes.size() - 1, nearestIdx + rangeCount);
     ...
     * @return CalculationResult
     */
    static CalculationResult calculateFixedDifference(double basePrice, double strikeDiff, int rangeCount = 0) {
        CalculationResult result;
        if (strikeDiff <= 0 || basePrice <= 0) return result;
