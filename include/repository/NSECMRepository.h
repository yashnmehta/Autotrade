...
public:
     ...
     */
    void updateOHLC(int64_t token, double open, double high, double low, double close);
...
private:
    ...
    // Level 2: Parallel arrays (compact, no gaps)
    QVector<int64_t> m_token;           // Token for each contract (needed for reverse lookup)
