 ...
 * 
 * Use case: UI filtering where users select symbol → expiry → view sorted strikes
 */
...
protected:
    ...
    QHash<QString, QVector<int64_t>> m_symbolIndex;    // Symbol -> [tokens] (sorted by date/strike)
    QHash<QString, QVector<int64_t>> m_seriesIndex;    // Series -> [tokens] (sorted by date/symbol/strike)
    QHash<QString, QVector<int64_t>> m_expiryIndex;    // Expiry -> [tokens] (sorted by symbol/strike)
