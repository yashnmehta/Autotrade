...
public:
  ...
  /**
   * @brief Enable native C++ callbacks for ultra-low latency updates (65ns vs
   * 15ms)
   *
   ...
   *
   * Thread Safety: Callbacks are invoked from the same thread as updatePrice()
   * calls
   */
  ...
  const GenericTableProfile &getColumnProfile() const {
    return m_columnProfile;
  }
  ...
  void addScrips(const QList<ScripData> &scrips);
  ...
  void updateGreeks(int row, double iv, double bidIV, double askIV,
  ...
  /**
   * @brief Batch-update all fields from a single UDP tick — emits ONE
   * dataChanged
   *
   ...
   */
  void updateFromUdpTick(int row, const UDP::MarketTick &tick, double change,
...
private:
  ...
  QString formatColumnData(const ScripData &scrip,
                           MarketWatchColumn column) const;
};
