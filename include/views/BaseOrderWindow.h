...
public:
  ...
  void loadFromOrder(
      const XTS::Order &order); ///< Load from existing order for modification
  void
  loadFromOrders(const QVector<XTS::Order>
                     &orders); ///< Load multiple orders for batch modification

  void setScripDetails(const QString &exchange, int token,
  ...
  void resetToNewOrderMode(
      bool fastMode = false); ///< Reset window to new order mode (fastMode
                              ///< skips UI updates for hidden windows)
...
protected:
  ...
  void setModifyMode(bool enabled); ///< Configure UI for modify mode (disable
                                    ///< immutable fields)
