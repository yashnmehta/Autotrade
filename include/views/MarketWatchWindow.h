 ...
 *
 * Implements IMarketWatchViewCallback for ultra-low latency updates (65ns vs
 * 15ms).
 *
 * Features:
...
public:
   ...
   * @return WindowContext with contract and market data, or invalid context if
   * no selection
   */
   ...
   */
  void updateOHLC(int token, double open, double high, double low,
                  double close);
   ...
   *
   * Performance: 10-50ns (direct viewport invalidation) vs 500ns-15ms (Qt
   * signal)
   *
...
protected:
   ...
   */
  void performRowMoveByTokens(const QList<int> &tokens,
