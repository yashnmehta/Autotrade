...
/**
 * @brief Manages pre-cached Buy/Sell/SnapQuote windows for fast opening (~10ms
 * instead of ~400ms)
 *
 * This singleton class handles window pre-creation and reuse to dramatically
 * improve window open performance. It keeps existing MainWindow code clean by
 * managing all caching logic separately.
 */
...
public:
  ...
  static constexpr int CACHED_SNAPQUOTE_COUNT = 3;
...
private:
  ...
  bool m_buyWindowNeedsReset =
      true; ///< True if Buy window was closed (needs reset on next show)
  bool m_sellWindowNeedsReset =
      true; ///< True if Sell window was closed (needs reset on next show)
  ...
  // In-memory cache for window position (avoids slow QSettings read on every
  // F1/F2)
