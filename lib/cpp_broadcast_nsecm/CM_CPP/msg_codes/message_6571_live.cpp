...
namespace Message6571Receiver {
    ...
    std::atomic<int64_t> message6571Count{0};
    std::atomic<int64_t> message6571Saved{0};
    
    // CSV file for 6571
    std::ofstream csvFile6571;
...
using namespace Message6571Receiver;
...
bool initialize6571CSV() {
    ...
    std::string filename = "csv_output/message_6571_" + timestamp + ".csv";
    
    csvFile6571.open(filename);
    if (!csvFile6571.is_open()) {
        std::cerr << "❌ Failed to create CSV file: " << filename << std::endl;
        return false;
    }
    
    // Write headers
    csvFile6571 << "Timestamp,TransactionCode,BranchNumber,BrokerNumber,ActionCode,TraderWsBit,MsgLength,Message\n";
    csvFile6571.flush();
    
    std::cout << "📁 Created CSV file for Message 6571: " << filename << std::endl;
    return true;
}
...
void exportTo6571CSV(uint16_t transactionCode, uint16_t branchNumber, 
    ...
    csvFile6571 << getCurrentTimestamp() << ","
                << transactionCode << ","
                << branchNumber << ","
                << brokerNumber << ","
                << actionCode << ","
...
void process6571Message(const uint8_t* data, int dataLen) {
    if (dataLen < 298) { // 40-byte header + 258-byte message
        return;
    }
    
    message6571Count++;
    int64_t currentCount = message6571Count.load();
    ...
    if (currentCount == 1) {
        std::cout << "\n✅ First Message 6571 (BC_NORMAL_MKT_PREOPEN_ENDED) received\n" << std::endl;
    }
    
    // Export to CSV
    exportTo6571CSV(transactionCode, branchNumber, brokerNumber, actionCode, 
...
bool processUDPPacket6571(const uint8_t* data, int dataLen) {
    ...
    process6571Message(processData.data());
    return true;
}
...
void printStats6571() {
    ...
    int64_t msg6571 = message6571Count.load();
    int64_t saved = message6571Saved.load();
    ...
    if (seconds > 0) {
        ...
        std::cout << "⏱️  " << std::fixed << std::setprecision(0) << seconds 
                  ...
                  << compressed << " compressed | 🎯 6571: " << status 
                  << " | " << msg6571 << " msgs, " << saved << " saved" << std::endl;
    }
}

std::string formatNumber6571(int64_t n) {
    if (n < 1000) {
...
std::string getMessageCodeDescription6571(uint16_t code) {
    switch (code) {
        case 6511:
...
void printFinalStats6571() {
    ...
    std::cout << "FINAL STATISTICS - MESSAGE 6571 DECODER (BC_NORMAL_MKT_PREOPEN_ENDED)" << std::endl;
    std::cout << std::string(80, '=') << std::endl << std::endl;
    
    std::cout << "📊 LISTENER PERFORMANCE" << std::endl;
    std::cout << "  Runtime:              " << std::fixed << std::setprecision(0) 
              << seconds << " seconds" << std::endl;
    std::cout << "  Total Packets:        " << formatNumber6571(packets) << std::endl;
    ...
    std::cout << "  Compressed Packets:   " << formatNumber6571(compressed);
    if (packets > 0) {
        std::cout << " (" << std::setprecision(1) 
                  << (static_cast<double>(compressed) * 100.0 / packets) << "%)";
    }
    std::cout << std::endl;
    std::cout << "  Decompressed OK:      " << formatNumber6571(decompressed) << std::endl;
    std::cout << "  Decompression Errors: " << formatNumber6571(errors) << std::endl;
    if (compressed > 0) {
        std::cout << "  Success Rate:         " << std::setprecision(1) 
                  << (static_cast<double>(decompressed) * 100.0 / compressed) << "%" << std::endl;
    }
    
    std::cout << std::endl << "🎯 MESSAGE 6571 STATISTICS (BC_NORMAL_MKT_PREOPEN_ENDED)" << std::endl;
    std::cout << "  Total Messages:       " << formatNumber6571(msg6571) << std::endl;
    std::cout << "  Messages Saved:       " << formatNumber6571(saved) << std::endl;
    ...
    if (!messageCodeCounts.empty()) {
        ...
        for (const auto& cf : sorted) {
            std::string description = getMessageCodeDescription6571(cf.first);
            std::cout << std::left << std::setw(8) << cf.first << " " 
                      << std::setw(40) << description << " " 
                      << formatNumber6571(cf.second) << std::endl;
        }
    }
    ...
    std::cout << "  Messages: " << formatNumber6571(saved) << std::endl;
    std::cout << "  Format: Preopen ended notifications" << std::endl;
    
    std::cout << std::endl << std::string(80, '=') << std::endl;
    if (msg6571 > 0) {
        std::cout << "✅ SUCCESS: Preopen Ended Messages (6571) processing completed" << std::endl;
        std::cout << "📊 Captured " << saved << " preopen transition notifications" << std::endl;
    } else {
        std::cout << "⚠️  WARNING: No Preopen Ended Messages (6571) found during session" << std::endl;
        std::cout << "💡 Note: Preopen ended messages are broadcast at market transition (9:15 AM)" << std::endl;
    }
    std::cout << "✅ Check csv_output/ for message_6571_*.csv file" << std::endl;
    std::cout << std::string(80, '=') << std::endl;
}
...
bool runMessage6571Receiver(const std::string& multicastIP, int port) {
    ...
    std::cout << "\n" << std::string(80, '=') << std::endl;
    std::cout << "NSE CM UDP Receiver - Message 6571 (BC_NORMAL_MKT_PREOPEN_ENDED)" << std::endl;
    std::cout << std::string(80, '=') << std::endl;
    std::cout << "Listening for message code 6571 (0x19AB in hex)" << std::endl;
    std::cout << "Purpose: Preopen period ended notification" << std::endl;
    std::cout << "Session: Preopen → Regular transition (9:15 AM)" << std::endl;
    std::cout << "Multicast: " << multicastIP << ":" << port << std::endl;
    std::cout << "Press Ctrl+C to stop" << std::endl;
    std::cout << std::string(80, '=') << std::endl << std::endl;
    
    // Initialize CSV
    if (!initialize6571CSV()) {
    ...
    while (!shutdownFlag) {
        ...
        if (ready > 0 && FD_ISSET(sockfd, &readfds)) {
            ...
            if (n > 0) {
                packetCount++;
                totalBytes += n;
                processUDPPacket6571(reinterpret_cast<uint8_t*>(buffer), n);
            }
        }
    }
