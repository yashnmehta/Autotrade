...
namespace Message6511Receiver {
    ...
    std::atomic<int64_t> message6511Count{0};
    std::atomic<int64_t> message6511Saved{0};
    
    // CSV file for 6511
    std::ofstream csvFile6511;
...
using namespace Message6511Receiver;
...
bool initialize6511CSV() {
    ...
    std::string filename = "csv_output/message_6511_" + timestamp + ".csv";
    
    csvFile6511.open(filename);
    if (!csvFile6511.is_open()) {
        std::cerr << "❌ Failed to create CSV file: " << filename << std::endl;
        return false;
    }
    
    // Write headers
    csvFile6511 << "Timestamp,TransactionCode,BranchNumber,BrokerNumber,ActionCode,TraderWsBit,MsgLength,Message\n";
    csvFile6511.flush();
    
    std::cout << "📁 Created CSV file for Message 6511: " << filename << std::endl;
    return true;
}
...
void exportTo6511CSV(uint16_t transactionCode, uint16_t branchNumber, 
    ...
    csvFile6511 << getCurrentTimestamp() << ","
                << transactionCode << ","
                << branchNumber << ","
                << brokerNumber << ","
                << actionCode << ","
...
void process6511Message(const uint8_t* data, int dataLen) {
    if (dataLen < 298) { // 40-byte header + 258-byte message
        return;
    }
    
    message6511Count++;
    int64_t currentCount = message6511Count.load();
    ...
    if (currentCount == 1) {
        std::cout << "\n✅ First Message 6511 (BC_OPEN_MESSAGE) received\n" << std::endl;
    }
    
    // Export to CSV
    exportTo6511CSV(transactionCode, branchNumber, brokerNumber, actionCode, 
...
bool processUDPPacket6511(const uint8_t* data, int dataLen) {
    ...
    process6511Message(processData.data());
    return true;
}
...
void printStats6511() {
    ...
    int64_t msg6511 = message6511Count.load();
    int64_t saved = message6511Saved.load();
    ...
    if (seconds > 0) {
        ...
        std::cout << "⏱️  " << std::fixed << std::setprecision(0) << seconds 
                  ...
                  << compressed << " compressed | 🎯 6511: " << status 
                  << " | " << msg6511 << " msgs, " << saved << " saved" << std::endl;
    }
}
...
void printFinalStats6511() {
    ...
    std::cout << "FINAL STATISTICS - MESSAGE 6511 DECODER (BC_OPEN_MESSAGE)" << std::endl;
    std::cout << std::string(80, '=') << std::endl << std::endl;
    
    std::cout << "📊 LISTENER PERFORMANCE" << std::endl;
    std::cout << "  Runtime:              " << std::fixed << std::setprecision(0) 
              << seconds << " seconds" << std::endl;
    std::cout << "  Total Packets:        " << formatNumber6511(packets) << std::endl;
    ...
    std::cout << "  Compressed Packets:   " << formatNumber6511(compressed);
    if (packets > 0) {
        std::cout << " (" << std::setprecision(1) 
                  << (static_cast<double>(compressed) * 100.0 / packets) << "%)";
    }
    std::cout << std::endl;
    std::cout << "  Decompressed OK:      " << formatNumber6511(decompressed) << std::endl;
    std::cout << "  Decompression Errors: " << formatNumber6511(errors) << std::endl;
    if (compressed > 0) {
        std::cout << "  Success Rate:         " << std::setprecision(1) 
                  << (static_cast<double>(decompressed) * 100.0 / compressed) << "%" << std::endl;
    }
    
    std::cout << std::endl << "🎯 MESSAGE 6511 STATISTICS (BC_OPEN_MESSAGE)" << std::endl;
    std::cout << "  Total Messages:       " << formatNumber6511(msg6511) << std::endl;
    std::cout << "  Messages Saved:       " << formatNumber6511(saved) << std::endl;
    ...
    if (!messageCodeCounts.empty()) {
        ...
        for (const auto& cf : sorted) {
            std::string description = getMessageCodeDescription6511(cf.first);
            std::cout << std::left << std::setw(8) << cf.first << " " 
                      << std::setw(40) << description << " " 
                      << formatNumber6511(cf.second) << std::endl;
        }
    }
    ...
    std::cout << "  Messages: " << formatNumber6511(saved) << std::endl;
    std::cout << "  Format: Market open notifications" << std::endl;
    
    std::cout << std::endl << std::string(80, '=') << std::endl;
    if (msg6511 > 0) {
        std::cout << "✅ SUCCESS: Market Open Messages (6511) processing completed" << std::endl;
        std::cout << "📊 Captured " << saved << " market open notifications" << std::endl;
    } else {
        std::cout << "⚠️  WARNING: No Market Open Messages (6511) found during session" << std::endl;
        std::cout << "💡 Note: Market open messages are broadcast at market start (9:15 AM)" << std::endl;
    }
    std::cout << "✅ Check csv_output/ for message_6511_*.csv file" << std::endl;
    std::cout << std::string(80, '=') << std::endl;
}
...
bool runMessage6511Receiver(const std::string& multicastIP, int port) {
    ...
    std::cout << "\n" << std::string(80, '=') << std::endl;
    std::cout << "NSE CM UDP Receiver - Message 6511 (BC_OPEN_MESSAGE)" << std::endl;
    std::cout << std::string(80, '=') << std::endl;
    std::cout << "Listening for message code 6511 (0x1977 in hex)" << std::endl;
    std::cout << "Purpose: Market open notification" << std::endl;
    std::cout << "Session: Regular Market (9:15 AM start)" << std::endl;
    std::cout << "Multicast: " << multicastIP << ":" << port << std::endl;
    std::cout << "Press Ctrl+C to stop" << std::endl;
    std::cout << std::string(80, '=') << std::endl << std::endl;
    
    // Initialize CSV
    if (!initialize6511CSV()) {
    ...
    while (!shutdownFlag) {
        ...
        if (ready > 0 && FD_ISSET(sockfd, &readfds)) {
            ...
            if (n > 0) {
                ...
                processUDPPacket6511(reinterpret_cast<uint8_t*>(buffer), n);
            }
        }
    }
