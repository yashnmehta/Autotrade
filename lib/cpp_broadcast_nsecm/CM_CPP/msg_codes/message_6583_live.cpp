...
namespace Message6583Receiver {
    ...
    std::atomic<int64_t> message6583Count{0};
    std::atomic<int64_t> message6583Saved{0};
    
    // CSV file for 6583
    std::ofstream csvFile6583;
...
using namespace Message6583Receiver;
...
bool initialize6583CSV() {
    ...
    std::string filename = "csv_output/message_6583_" + timestamp + ".csv";
    
    csvFile6583.open(filename);
    if (!csvFile6583.is_open()) {
        std::cerr << "❌ Failed to create CSV file: " << filename << std::endl;
        return false;
    }
    
    // Write headers
    csvFile6583 << "Timestamp,TransactionCode,BranchNumber,BrokerNumber,ActionCode,TraderWsBit,MsgLength,Message\n";
    csvFile6583.flush();
    
    std::cout << "📁 Created CSV file for Message 6583: " << filename << std::endl;
    return true;
}
...
void exportTo6583CSV(uint16_t transactionCode, uint16_t branchNumber, 
    ...
    csvFile6583 << getCurrentTimestamp() << ","
                << transactionCode << ","
                << branchNumber << ","
                << brokerNumber << ","
                << actionCode << ","
...
void process6583Message(const uint8_t* data, int dataLen) {
    if (dataLen < 298) { // 40-byte header + 258-byte message
        return;
    }
    
    message6583Count++;
    int64_t currentCount = message6583Count.load();
    ...
    if (currentCount == 1) {
        std::cout << "\n✅ First Message 6583 (BC_CLOSING_START) received\n" << std::endl;
    }
    
    // Export to CSV
    exportTo6583CSV(transactionCode, branchNumber, brokerNumber, actionCode, 
...
bool processUDPPacket6583(const uint8_t* data, int dataLen) {
    ...
    process6583Message(processData.data());
    return true;
}
...
void printStats6583() {
    ...
    int64_t msg6583 = message6583Count.load();
    int64_t saved = message6583Saved.load();
    ...
    if (seconds > 0) {
        ...
        std::cout << "⏱️  " << std::fixed << std::setprecision(0) << seconds 
                  ...
                  << compressed << " compressed | 🎯 6583: " << status 
                  << " | " << msg6583 << " msgs, " << saved << " saved" << std::endl;
    }
}

std::string formatNumber6583(int64_t n) {
    if (n < 1000) {
...
void printFinalStats6583() {
    ...
    std::cout << "FINAL STATISTICS - MESSAGE 6583 DECODER (BC_CLOSING_START)" << std::endl;
    std::cout << std::string(80, '=') << std::endl << std::endl;
    
    std::cout << "📊 LISTENER PERFORMANCE" << std::endl;
    std::cout << "  Runtime:              " << std::fixed << std::setprecision(0) 
              << seconds << " seconds" << std::endl;
    std::cout << "  Total Packets:        " << formatNumber6583(packets) << std::endl;
    ...
    std::cout << "  Compressed Packets:   " << formatNumber6583(compressed);
    if (packets > 0) {
        std::cout << " (" << std::setprecision(1) 
                  << (static_cast<double>(compressed) * 100.0 / packets) << "%)";
    }
    std::cout << std::endl;
    std::cout << "  Decompressed OK:      " << formatNumber6583(decompressed) << std::endl;
    std::cout << "  Decompression Errors: " << formatNumber6583(errors) << std::endl;
    if (compressed > 0) {
        std::cout << "  Success Rate:         " << std::setprecision(1) 
                  << (static_cast<double>(decompressed) * 100.0 / compressed) << "%" << std::endl;
    }
    
    std::cout << std::endl << "🎯 MESSAGE 6583 STATISTICS (BC_CLOSING_START)" << std::endl;
    std::cout << "  Total Messages:       " << formatNumber6583(msg6583) << std::endl;
    std::cout << "  Messages Saved:       " << formatNumber6583(saved) << std::endl;
    
    std::cout << std::endl << "📁 CSV FILE CREATED" << std::endl;
    std::cout << std::string(80, '-') << std::endl;
    std::cout << "  Location: csv_output/" << std::endl;
    std::cout << "  Messages: " << formatNumber6583(saved) << std::endl;
    std::cout << "  Format: Closing session start notifications" << std::endl;
    
    std::cout << std::endl << std::string(80, '=') << std::endl;
    if (msg6583 > 0) {
        std::cout << "✅ SUCCESS: Closing Session Start Messages (6583) processing completed" << std::endl;
        std::cout << "📊 Captured " << saved << " closing session start notifications" << std::endl;
    } else {
        std::cout << "⚠️  WARNING: No Closing Session Start Messages (6583) found during session" << std::endl;
        std::cout << "💡 Note: Closing start messages are broadcast at closing session beginning" << std::endl;
    }
    std::cout << "✅ Check csv_output/ for message_6583_*.csv file" << std::endl;
    std::cout << std::string(80, '=') << std::endl;
}
...
bool runMessage6583Receiver(const std::string& multicastIP, int port) {
    ...
    std::cout << "\n" << std::string(80, '=') << std::endl;
    std::cout << "NSE CM UDP Receiver - Message 6583 (BC_CLOSING_START)" << std::endl;
    std::cout << std::string(80, '=') << std::endl;
    std::cout << "Listening for message code 6583 (0x19C7 in hex)" << std::endl;
    std::cout << "Purpose: Closing session started notification" << std::endl;
    ...
    while (!shutdownFlag) {
        ...
        if (ready > 0 && FD_ISSET(sockfd, &readfds)) {
            ...
            if (n > 0) {
                packetCount++;
                totalBytes += n;
                processUDPPacket6583(reinterpret_cast<uint8_t*>(buffer), n);
            }
        }
    }
