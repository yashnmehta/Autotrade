...
void parse_message_6522(const BCAST_VCT_MESSAGES* msg) {
    ...
    // Create post-close message data (reusing MarketOpenMessage structure)
    MarketOpenMessage postCloseMsg;
    postCloseMsg.txCode = txCode;
    postCloseMsg.timestamp = logTime;
    postCloseMsg.symbol = symbol;
    postCloseMsg.series = series;
    postCloseMsg.marketType = marketType;
    postCloseMsg.message = broadcastMsg;
    
    // Dispatch callback (using the same callback as market open/close/status changes)
    MarketDataCallbackRegistry::instance().dispatchMarketOpen(postCloseMsg);
    
    // Debug output (optional, can be removed in production)
    #ifdef DEBUG_PARSER_6522
    std::cout << "[6522] Post-Close Message:" << std::endl;
