...
void parse_uncompressed_message(const char *data, int16_t length) {
  ...
  case TxCodes::BCAST_INDICES:
    if (length >= sizeof(MS_BCAST_INDICES)) {
      ...
      // in this case messages have less than 6 records so we should parse it
      // accordingly.
      parse_message_7207_partial(
          reinterpret_cast<const MS_BCAST_INDICES *>(data));
    }
    break;
  ...
  case TxCodes::BC_SYMBOL_STATUS_CHANGE_ACTION:
    if (length >= sizeof(BC_SYMBOL_STATUS_CHANGE_ACTION)) {
      parse_symbol_status_change(
          reinterpret_cast<const BC_SYMBOL_STATUS_CHANGE_ACTION *>(data));
    }
    break;
  ...
  case TxCodes::BCAST_TURNOVER_EXCEEDED:
    if (length >= sizeof(MS_BCAST_TURNOVER_EXCEEDED)) {
      parse_message_9010(
          reinterpret_cast<const MS_BCAST_TURNOVER_EXCEEDED *>(data));
    }
    break;
