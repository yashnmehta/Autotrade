...
void Config::loadFromEnv() {
    ...
    if ((env_val = std::getenv("NSECM_MULTICAST_IP"))) {
        multicast_ip = env_val;
    }
    if ((env_val = std::getenv("NSECM_PORT"))) {
        port = std::stoi(env_val);
    }
    if ((env_val = std::getenv("NSECM_BUFFER_SIZE"))) {
        buffer_size = std::stoul(env_val);
    }
    if ((env_val = std::getenv("NSECM_LOG_LEVEL"))) {
        log_level = env_val;
    }
    if ((env_val = std::getenv("NSECM_LOG_FILE"))) {
        log_file = env_val;
    }
    if ((env_val = std::getenv("NSECM_STATS_INTERVAL"))) {
