...
void OptionChainWindow::refreshData()
{
    ...
    qDebug() << "[OptionChain] Total contracts from repo:" << contracts.size() 
    ...
    qDebug() << "[OptionChain] After filtering - Strikes:" << strikes.size() 
             << "Calls:" << callContracts.size() << "Puts:" << putContracts.size();
    
    if (strikes.isEmpty()) {
         qDebug() << "[OptionChain] ⚠ No strikes found!";
         return;
    }
    ...
    qDebug() << "[OptionChain] Subscribing to" << sortedStrikes.size() << "strikes for symbol:" << m_currentSymbol;
    ...
    for (double strike : sortedStrikes) {
        ...
        if (callContracts.contains(strike)) {
            ...
            qDebug() << "[OptionChain] Subscribing Call - Strike:" << strike 
                     << "Token:" << data.callToken << "Segment:" << exchangeSegment;
        ...
        if (putContracts.contains(strike)) {
            ...
            qDebug() << "[OptionChain] Subscribing Put - Strike:" << strike 
                     << "Token:" << data.putToken << "Segment:" << exchangeSegment;
...
void OptionChainWindow::onTickUpdate(const UDP::MarketTick &tick)
{
    ...
    if (tickCount++ < 10) {
        qDebug() << "[OptionChain] UDP Tick received - Token:" << tick.token 
                 << "LTP:" << tick.ltp << "Segment:" << (int)tick.exchangeSegment;
    }
    
    if (!m_tokenToStrike.contains(tick.token)) {
        static int unmappedCount = 0;
        if (unmappedCount++ < 5) {
            qDebug() << "[OptionChain] ⚠ Token not mapped:" << tick.token 
                     << "Available tokens:" << m_tokenToStrike.size();
        }
        return;
    }
